...
The workspace home and secrets directory are owned by the workspace user inside the container and are not
reachable from the host filesystem. Reach them over SSH as the workspace user, or with
`docker compose exec workspaces`.
...
| --- | --- | --- | --- |
| Inbound | `workspaces/ssh/keys/<module>/id_ed25519` on the host | container `authorized_keys` | The CLI, Fabric and Cursor Remote SSH signing in to the workspace |
...
├── ssh/                     # Every private key, gitignored
│   ├── sshd_config          # SSH daemon config (port 22 → 2222)
│   ├── config               # Generated SSH config include file
│   └── keys/
│       ├── ssh_host_*_key   # The container's own host keys
│       └── <module>/        # The control side's key for signing in to that workspace
├── templates/
...
`ssh/keys/<module>/` because Fabric, `ssh` and Cursor all read it from the host, and a volume would
put it behind root under `/var/lib/docker`.
...
`workspaces/ssh/keys/<workspace-module>/` so Cursor, Fabric, and local SSH tooling can use it.
...
```bash
ssh -i workspaces/ssh/keys/myproject/id_ed25519 myproject@localhost -p 2222
```
...
Host myproject-workspace
    ...
    Port 2222
    User myproject
    IdentityFile /absolute/path/to/just_build/workspaces/ssh/keys/myproject/id_ed25519
```
