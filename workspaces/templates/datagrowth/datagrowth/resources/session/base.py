...
class SessionResource(Resource[MainResourceSignatureType], Generic[MainResourceType, MainResourceSignatureType]):
    ...
    def prepare_session_step_inputs(self, step: ResourceStep, args: tuple[Any, ...], kwargs: dict[str, Any]) -> tuple[tuple[Any, ...], dict[str, Any]]:
        """
        Prepare inputs for a session step using state collected by earlier steps.
        Override this method to pass data from the Session to a later step.
        """
        return args, kwargs

    def prepare_session_step_signature(self, step: ResourceStep, signature: Signature) -> Signature:
        """
        Prepare a session step signature using state collected by earlier steps.
        Override this method to add auth data without exposing it as step inputs.
        """
        return signature
    ...
    def extract_session(self, *args: Any, **kwargs: Any) -> Session:
        ...
        for ix, step in enumerate(self.SESSION_STEPS):
            ...
            try:
                step_args, step_kwargs = self.prepare_session_step_inputs(step, args, kwargs)
                initial_signature = step_resource.prepare_extract(*step_args, **step_kwargs)
                step_signature = self.prepare_session_step_signature(step, initial_signature)
                if step_signature is initial_signature:
                    step_resource = step_resource.extract(*step_args, **step_kwargs)
                else:
                    step_resource.signature = step_signature
                    step_resource = step_resource.extract()
                step_resource.close()
